#!/usr/bin/env bash
# traffic 主菜单显示与 case 映射 / ui_item set -e 回归（mock，不触碰真实 tc）
set -Eeuo pipefail
...
ui_rc=$?
set -e
assert "ui_item multi no-subtitle exit 0" '[[ $ui_rc -eq 0 ]]'
assert "ui_item multi no-subtitle survived" '[[ "$ui_out" == *SURVIVED* ]]'
assert "ui_item printed first item" '[[ "$ui_out" == *"安装流量监控"* ]]'
assert "ui_item printed last item" '[[ "$ui_out" == *"返回"* ]]'
