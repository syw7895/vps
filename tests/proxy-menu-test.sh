...
assert "prompt present" '[[ "$out" == *"请选择 [0-3]"* ]]'

out3=$(printf '3\n0\n' | main_menu 2>&1) || true
assert "case 3 menu_uninstall" '[[ "$out3" == *"__CALL_menu_uninstall__"* ]]'
assert "case 3 not install" '[[ "$out3" != *"__CALL_menu_install__"* ]]'
