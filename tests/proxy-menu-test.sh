...
assert "ip direct cert pin in link" 'grep -q "pinnedPeerCertSha256=\${WS_PIN}" "$PROXY"'
assert "ip link no allowInsecure" '! grep -q "allowInsecure=1" "$PROXY"'
assert "ip link no fingerprint param" '! grep -qE "type=ws&[^\"#]*fp=" "$PROXY"'
assert "ip cert sha256 helper" 'grep -q "^ws_cert_sha256()" "$PROXY"'
assert "ip direct no fake host sni" '! grep -q "host=\${WS_SNI}" "$PROXY" && ! grep -q "Host/SNI:" "$PROXY"'
assert "ip cert uses IP SAN" 'grep -q "subjectAltName=IP:" "$PROXY"'
assert "ip mode no SNI preset" '! grep -q "WS_SNI=\${SNI_PRESETS" "$PROXY"'
