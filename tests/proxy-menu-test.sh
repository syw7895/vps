...
assert "no cloudflare install menu item" '! grep -q "ui_item 4 \"安装 / 更新 VLESS + WS + TLS（Cloudflare CDN）\"" "$PROXY"'
assert "install menu range stops at 3" 'grep -q "请选择 \[0-3\]" "$PROXY"'
assert "ws random port prompt" 'grep -q "输入 random=随机" "$PROXY"'
assert "direct port defaults random" 'grep -q "TLS 端口（空=随机；输入 random=随机）" "$PROXY"'
assert "domain prompt has no old default" 'grep -q "domain=\$(prompt \"域名（已解析到本机）\" \"\")" "$PROXY"'
assert "direct uses domain without address prompt" 'grep -q "server=\$domain" "$PROXY" && ! grep -q "直连地址（空=域名）" "$PROXY"'
